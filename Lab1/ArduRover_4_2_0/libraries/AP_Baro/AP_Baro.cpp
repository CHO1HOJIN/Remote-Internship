...
const AP_Param::GroupInfo AP_Baro::var_info[] = {
    ...
    AP_GROUPINFO("_FAIL",  1, AP_Baro, baro_fail,  0),
...
float AP_Baro::get_external_temperature(const uint8_t instance) const
{
    ...
    if (airspeed != nullptr) {
        float temperature;
        if (airspeed->healthy() && airspeed->get_temperature(temperature) && !baro_fail) {
            return temperature;
        }
    }
#endif
...
void AP_Baro::update(void)
{
    ...
    for (uint8_t i=0; i<_num_sensors; i++) {
        if (sensors[i].healthy && !baro_fail) {
    ...
    if (healthy() && !baro_fail) {
        _climb_rate_filter.update(get_altitude(), get_last_update());
    }

    // choose primary sensor
    if (_primary_baro >= 0 && _primary_baro < _num_sensors && healthy(_primary_baro) && !baro_fail) {
